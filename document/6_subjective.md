...
We show that within the scope of a number of evaluated task types and domains, personalized crowdsourcing is feasible.
...
Our primary goal in evaluating taste-matching and taste-grokking for various problems is to compare the space of possibility for personalized crowdsourcing: what works, what does not, and when.
Beyond the underlying philosophies of 'matching' or 'grokking' being compared in the two protocols, we touch on training set size and selection, tasks of different complexity and in different domains, contribution granularity,
...
The work we present in this paper builds on existing crowdsourcing research to generalize previous approaches that have been taken to support subjective crowdsourcing.
In this section we provide an overview of relevant approaches in crowdsourcing, highlighting approaches that are particularly similar to the taste-matching and taste-grokking protocols we explore.

Crowdsourcing for subjective tasks is common in volunteer crowdsourcing settings.
...
The work we present differs from earlier work in providing a generalized treatment of personalized crowdsourcing,
...
Taste-matching has precedent in other areas, though our implementation differs in applying the concepts to on-demand needs,
...
For clarity, we use 'communication set'
...
In section XX, we compare the effect of purposively sampled profiling sets.
...
Figure XX1 illustrates a simple taste-matching setting.
...
In the settings evaluated for this paper, we try to base these decisions on precedents and realistic settings, but acknowledge that these are decisions rather than rules.
...
Taste-grokking workers were shown a profiling set of *10* items with the requester's opinions.
...
While we observe considerable variation across individuals, we find that people are consistently similar to each other over time.
To show this,
...
Table XX2 shows the average performance of taste-grokking when performed by any given worker, when aggregated from five worker contributions, and when aggregated from five workers that had been cross-validated as 'good grokkers' from a pool of thirty.
...
it is possible that some workers have particular difficulty with the task or some workers perform the work very carefully, with much cross-checking with the communication set.
...
but were also asked 'what is your reason for this rating?' We found that by asking workers to contemplate and explicate their reasoning for judgments, their behaviors changed.
...
For all tasks, an optional feedback form was provided to allow for workers to communicate with us.
...
However, it also frustrated workers when the communication set failed.
...
For example, one worker reported, “I think a few more rated pictures would have been helpful in helping me to decide some of the choices, because there wasn't anything that similar in the rated items to those items such as the cappuccino, burgers, hot chocolate.” The taste communication sets were selected randomly for each requester, and taste-grokking was tried with two different set per requester, but the salt and pepper shaker tasks appeared less likely to settle with a poor communication set.
This feedback seems to align with the poorer grokking results for cuisine, which will be discussed below.

Figure XX6 shows the performance of an all-worker aggregation for each individual taste-grokking experiment, in order to demonstrate the range of overall prediction quality related to different profiling sets.
For comparison, the worst optimized profiling set (described in next section) performance was RMSE=1.05.
Figure XX7 offers an example of a successful profiling set alongside one where workers performed poorly.
The reason the bottom set in Figure XX7 performed poorly is only speculative, though it is notable that it did not capture the workers opinion of 'cute figurines', as the one above did with the cuddling bird and the cupcake salt and pepper shakers.
The taste cluster examples in Figure XX8 show that this is a large facet of the space.
...
In the results presented, the items used for profiling and subsequently for communication in taste-grokking were selected randomly.
Random selection can potentially fail when there are many decision dimensions to communicate, as we observed for taste-grokking over cuisine.
...
For the next setting, we pursue this more complex use case.
...
The relatively short average length of reviews may limit this effect, but we expect a real-world setting would require a more robust similarity metric.
...
Despite our adoption of the term 'taste' in taste-matching and taste-grokking, there are many non-normative tasks that do not refer to subjective taste but nonetheless are requester-specific and may be suited for personalization.
Two additional areas may be characterized as those affected by *style* and *biases*.
By way of example, we performed a handwriting imitation study, looking at the ability of strangers to personalize text in a person's handwriting.
...
For simplicity, we demonstrated on a single phrase.
...
This obviates the need to rank overall similarity between users as we can simply use similarity between sample pairs.
...
We refer to the proportion of one-hundred evaluators that answered “Yes” to the question as the *score*.
...
This means that we could potentially evaluate workers on their ability to correctly distinguish authentic from forged handwriting.
...
Figure XX10 shows the score of the best of $k$ random samples in people's own handwriting.
Even for
$k$=13, this number is below 0.50.
For this reason, we determined that matching was intractable.
...
For example, in the most drastic example, we found that a requester wrote the profiling phrase with a crossed 'z', but the test phrase without the line through it.
...
While we observed that they generally provided improvements over a non-personalized baseline,
...
We have contributed an initial set of observations,
...
An initial study of optimized item selection shows that purposive strategies can improve personalization, though our method required prior data for clustering; content-based strategies
...
While crowdsourcing has been notably applied to ground-truth tasks, this study turned the lens on a less-studied type of problem, one which we believe human workers are especially well-positioned to address: subjective, person-specific tasks.
We discuss *personalized crowdsourcing* as an additional facet of crowdsourcing, one that is well-suited to on-demand personalization.
